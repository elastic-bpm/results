...
options(scipen=999)
experiments <- c("C") # experiments <- c("A", "B", "C", "D", "E", "F", "G", "H")
numruns <- c(2) # numruns <- c(8,8,2,8,8,8,2,10)
...
for(i in 1:length(experiments)) 
{
  ...
  colnames(results) <- c("50","100")
  legend_texts = expression("T"["m"], "T"["w"], "T"["dh"], "T"["h"], "T"["ds"], "T"["s"])
  
  # percentages 
  prop = prop.table(results,margin=2)
  par(mar=c(5, 5, 5, 5), xpd=TRUE)
  barplot(prop, col=gray.colors(length(rownames(prop)), start = 0.3, end = 0.9, gamma = 2.2, alpha = NULL), width=2, beside=FALSE, xlab="# Workflows", ylab="relative time spent")
  ...
  # counts
  par(mar=c(5, 5, 5, 5), xpd=TRUE)
  barplot(results, col=gray.colors(length(rownames(results)), start = 0.3, end = 0.9, gamma = 2.2, alpha = NULL), width=2, beside=TRUE, xlab="# Workflows", ylab="average runtime in minutes")
