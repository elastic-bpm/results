...
options(scipen=999)
experiments <- c("B") # experiments <- c("A", "B", "C", "D", "E", "F", "G", "H")
...
for(i in 1:length(experiments)) 
{
  ...
  for(j in 1:numruns){
    ...
    load("wfDF.RData")
    x <- data.matrix(colAvg(wfDF))
    x <- sweep(x, 1, c(1000,1000,1000,1000,1000,1000,1000,1000), "/")
  ...
  colnames(results) <- c("1","3","5","7","9","11","13","15")
  legend_texts = expression("T"["m"], "T"["w"], "T"["dh"], "T"["h"], "T"["ds"], "T"["s"])
  
  # percentages 
  prop = prop.table(results,margin=2)
  par(mar=c(5, 5, 5, 5), xpd=TRUE)
  barplot(prop, col=gray.colors(length(rownames(prop)), start = 0.3, end = 0.9, gamma = 2.2, alpha = NULL), width=2, beside=FALSE, xlab="# Machines", ylab="relative time spent")
  ...
  # counts
  par(mar=c(5, 5, 5, 5), xpd=TRUE)
  barplot(results, col=gray.colors(length(rownames(results)), start = 0.3, end = 0.9, gamma = 2.2, alpha = NULL), width=2, beside=TRUE, xlab="# Machines", ylab="average runtime in minutes")
  legend("topright",inset=c(0,0.05), fill=gray.colors(length(rownames(results)), start = 0.1, end = 0.9, gamma = 2.2, alpha = NULL), legend=legend_texts, ncol=6)
}
